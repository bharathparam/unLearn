"""Script to write the verification_engine.py page."""
code = '''"""Page 10 — Neural Verification Engine — connected to FastAPI backend."""
...
def _api_post(endpoint: str, payload: dict) -> dict | None:
    try:
        r = httpx.post(f"{API_BASE}{endpoint}", json=payload, timeout=30)
        r.raise_for_status()
        return r.json()
    except httpx.ConnectError:
        st.error("Cannot connect to Neural Verification Engine at localhost:8000. Is it running?")
        return None
...
def _api_get(endpoint: str) -> dict | None:
    try:
        ...
        st.error("Cannot connect to Neural Verification Engine at localhost:8000.")
        return None
    except Exception as e:
        st.error(f"API error: {e}")
        return None
...
with open('pages/verification_engine.py', 'w', encoding='utf-8') as f:
    f.write(code)
